...
class PatchTST_backbone(nn.Module):
    ...
    def forward(self, z, epoch_num = 0, batch_num = 0):                                                                   # z: [bs x nvars x seq_len]
        ...
        if self.padding_patch == 'end':
            z_cd = self.padding_patch_layer(z_cd)
        z_cd = z_cd.unfold(dimension=-1, size=self.patch_len, step=self.stride)                   # z: [bs x nvars x patch_num x patch_len]
        z_cd = z_cd.permute(0,1,3,2)                                                              # z: [bs x nvars x patch_len x patch_num]
